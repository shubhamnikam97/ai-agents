...
def main():
    parser = argparse.ArgumentParser(description="SQL Query Agent using LangChain")
    parser.add_argument("--db", default="demo.db", help="Path to SQLite database file")
    parser.add_argument("--question", help="Natural language question (omit for interaction)")
    parser.add_argument("--allow-write", action="store_true", 
                        help="Open the SQLite database read-write instead of read-only")
    args = parser.parse_args()
    
    if args.db == "demo.db" and not os.path.exists(args.db):
    ...
    if args.question:
        ...
        while True:
            ...
            if question.lower() in ("quit", "exist", "q"):
                break
            if not question:
                continue
