...
def build_email_crew(context: str, tone: str, recipient: str) -> str:
    ...
    analyst = Agent(
        ...
        backstory="You are an expert business communication analyst who distills complex solutions into clear email requirements",
        llm=llm,
        verbose=False,
    )
    ...
    writer = Agent(
        ...
        goal="Draft clear, concise, and effective professional email",
        backstory="You are a professional copywrite specializing in business email that get response",
        llm=llm,
        verbose=False,
    )

    analyze_task = Task(
        description=f"""Analyze the email requirements:
        Context: {context}
        Recipient: {recipient}
        Desired tone: {tone}
